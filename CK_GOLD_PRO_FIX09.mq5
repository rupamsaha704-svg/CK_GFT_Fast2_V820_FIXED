//+------------------------------------------------------------------+
//|  CK_GOLD_PRO_FIX09.mq5 — FIXED-LOT deploy build (always 0.09).    |
//|  Same validated v23 trend logic (HTF EMA200, breakout, EMA20      |
//|  pullback, SL swing+ATR, MaxSL_ATR 2.5, RR 3.0, BE@+1.5R, ATR     |
//|  shift-0, 3 trades/day, daily +/-R limits) + the 6 live-safety    |
//|  fixes. DIFFERENCE vs CK_GOLD_PRO: lot is NOT risk-based — every  |
//|  trade uses EXACTLY InpFixedLot (0.09), hard-locked. InpRiskPercent|
//|  now ONLY sets the daily +/-R stop reference, NOT the lot size.   |
//|  Note: with fixed lot, per-trade $ risk varies with SL distance.  |
//|  (Backtest 2.0%-ref was 100% capped at 0.09 already, so results   |
...
input double InpFixedLot          = 0.09;   // EXACT lot on every trade (hard-locked)
input double InpMaxLot           = 0.09;   // safety ceiling (never exceed)
input double InpRiskPercent      = 2.0;    // ONLY the daily +/-R stop reference now (not lot size)
...
int OnInit(){
   ...
   GK_DAY="ckgpf_day_"+scope; GK_BAL="ckgpf_bal_"+scope; GK_TRD="ckgpf_trd_"+scope;
   LoadOrResetDaily();
   PrintFormat("[CK_GOLD_PRO_FIX09] FIXED lot=%.2f (cap %.2f)  dailyR-ref risk=%.2f%%  maxSpreadPts=%d",
      FixedLot(), InpMaxLot, InpRiskPercent, (int)MathRound(InpMaxSpreadPrice/SymbolInfoDouble(_Symbol,SYMBOL_POINT)));
...
void CountIfFilled(bool sent,string tag){
   ...
   if(filled){ g_tradesToday++; g_beActivated=false; GlobalVariableSet(GK_TRD,g_tradesToday); }
      ...
      PrintFormat("[CK_GOLD_PRO_FIX09] ORDER_FAIL tag=%s sent=%s rc=%u %s deal=%I64u order=%I64u",
...
double OnTester(){
   ...
   int h=FileOpen("ck_gold_pro_fix09_trades.csv", FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI, ",");
   if(h!=INVALID_HANDLE){
