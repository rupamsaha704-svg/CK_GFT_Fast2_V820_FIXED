//+------------------------------------------------------------------+
//|  CK_GOLD_COMBO_v3.mq5 - FIX09 + DTREND + DONCHIANv3 + MR_Z         |
//|                                                                    |
//|  v3 CHANGES vs v2:                                                 |
//|    1) DONCHIAN upgraded (in-place): 5 new filters to cut losses    |
//|       - ADX regime filter (skip breakout in chop)                  |
//|       - Volatility filter (H1_ATR24 / D1_ATR10)                   |
//|       - Overnight hour block (avoid 00-02 gap losses)             |
//|       - Body confirmation (close in direction of breakout)         |
//|       - ATR trailing stop + opposite Donchian(10) exit             |
//|       (all NEW inputs DN_Use* default ON; A/B by turning OFF)      |
//|    2) NEW 4th sleeve MR_Z (magic 20260905):                        |
//|       - z-score mean reversion (z=|close-SMA20|/StdDev20)          |
//|       - ONLY when ADX < 22 (ranging market - opposite regime)      |
//|       - TP = SMA20 mean, SL = 1.5 ATR beyond entry                 |
//|       - Complements DONCHIAN which trades only when ADX >= 22      |
//|                                                                    |
//|  All 4 sleeves respect master guard: static DD + daily governor    |
//|  + 3% funded risk cap + combined SL-risk check across ALL magic    |
//|  numbers (FIX_Magic, DT_Magic, DN_Magic, MR_Magic).                |
//|                                                                    |
//|  HOW TO TEST (Strategy Tester):                                   |
//|    Symbol=XAUUSD  Timeframe=M15                                   |
//|    Deposit=6000  Leverage=1:100  Model=1min OHLC (fast) or real   |
//|    ticks. All four strategies share the $6000; margin is real.    |
//+------------------------------------------------------------------+
#property copyright "CK GOLD COMBO v3.1 PROFIT HANDOFF"
...
CTrade mr;   // MR_Z trader (v3 4th sleeve)
...
input bool   Combo_EnableMR       = false;  // v3.1 candidate: MR disabled until independent validation
input bool   Combo_UseProfitHandoff= true;   // v3.1: core entries evict satellites; satellites never overlap core
...
input double Combo_FundedFloatFlatPct  = 1.2;   // FUNDED: extra floating-loss backstop, conservative inside FN 3% total-risk rule
input double Combo_FundedPerTradePct   = 2.0;   // FUNDED FN rule-safe: per-trade cap; total predictive buffer remains 2.5% in funded preset (<3% hard)
input double Combo_FundedFixLot        = 0.01;  // FUNDED: FIX09 min lot; max-open=1 and total-risk buffer protect FN $180 line
...
input int    Combo_EvalMaxOpenTotal   = 0; // v3: EVAL max open positions (0 = off, no limit). Set to 1 to prevent overnight stacking disasters.
...
//====================== DONCHIAN inputs (DN_) ======================
// v3: 3rd sleeve - Turtle 20-bar close breakout on M15, symmetric long/short.
// FIVE NEW FILTERS (borrowed parts from CK_TREND_ATR_v1, CK_XAU_ASIAN_FADE, CK_GOLD_MR):
//   - ADX regime filter (only trade when trending, ADX >= DN_MinADX)
//   - Volatility filter (skip on H1_ATR24 / D1_ATR10 > DN_MaxVolMult days)
//   - Overnight hour block (avoid gap losses at Hour 00-02)
//   - Body confirmation (bar must close in the direction of breakout)
//   - ATR trailing stop + opposite Donchian(10) exit (replaces fixed 3R TP)
...
input double DN_TP_Rmultiple       = 3.0;            // TP = entry +/- 3R (used only when DN_UseTrailStop=false)
...
//--- v3 NEW: ADX regime filter (borrowed from CK_GOLD_MR: only trade breakout when trending) ---
input bool   DN_UseADXFilter       = true;           // v3: block breakout when market is ranging
input int    DN_ADXPeriod          = 14;             // v3: ADX period
input double DN_MinADX             = 22.0;           // v3: min ADX to allow breakout entry
//--- v3 NEW: Volatility filter (borrowed from CK_XAU_ASIAN_FADE: skip breakout during vol spikes) ---
input bool   DN_UseVolFilter       = true;           // v3: block entries during volatility spikes
input double DN_MaxVolMult         = 2.0;            // v3: skip if H1_ATR24 / D1_ATR10 > this
//--- v3 NEW: Overnight hour block (fix 6/10 big losses at Hour 01:00) ---
input string DN_BlockHours         = "0,1,2";        // v3: server hours to block DONCHIAN entries (comma-sep)
//--- v3 NEW: Body confirmation (bar must close in breakout direction, not just wick through) ---
input bool   DN_RequireBody        = true;           // v3: require breakout bar body to confirm direction
//--- v3 NEW: Trailing stop + opposite Donchian exit (borrowed from CK_TREND_ATR_v1) ---
input bool   DN_UseTrailStop       = false;          // v3.1 pinned: preserve fixed 3R runner; no continuous trail
input double DN_TrailATR           = 2.0;            // v3: trailing stop distance (ATR multiple)
input bool   DN_UseOppExit         = false;          // v3.1 pinned: no early opposite-channel exit
input int    DN_ExitLookback       = 10;             // v3: opposite channel lookback for early exit

//====================== MR_Z inputs (MR_) v3 4th sleeve =============
// v3: Mean-reversion sleeve, borrowed from CK_MR_StdDev_v1 (z-score entry) + CK_GOLD_MR (ADX regime).
// Trades ONLY when ADX < DN_MinADX (ranging) - the OPPOSITE regime from DONCHIAN's trending breakout.
// Full complementary coverage: DONCHIAN in trend, MR_Z in chop. Different magic 20260905.
input long   MR_Magic              = 20260905;       // v3: distinct from all other magics
input double MR_FixedLot           = 0.02;           // same lot as DONCHIAN for rule-safe combined risk
input int    MR_MaxTradesPerDay    = 2;              // conservative: 2 MR trades/day max
input ENUM_TIMEFRAMES MR_TF        = PERIOD_M15;     // same TF as DONCHIAN
input int    MR_MAPeriod           = 20;             // SMA20 = mean-reversion target
input double MR_EntryZ             = 2.0;            // enter when |z| >= 2.0 (2 std dev extreme)
input int    MR_ATRPeriod          = 14;
input double MR_SL_ATR             = 1.5;            // SL = 1.5 * ATR beyond entry (tight)
input double MR_MinTP_ATR          = 0.5;            // require TP (mean) at least this far, else skip
input double MR_MaxSL_ATR          = 3.0;            // reject too-wide stops
input double MR_MaxADX             = 22.0;           // ONLY trade when ADX < this (ranging regime)
input double MR_MaxSpreadPrice     = 0.60;
input int    MR_SessionStartHour   = 8;              // entries 08:00-22:00 server (same window as DONCHIAN)
input int    MR_SessionEndHour     = 22;
input bool   MR_BlockFridayEvening = true;
input int    MR_FridayBlockHour    = 20;
...
bool     g_blockHour[24];
int      g_handoffClosed=0, g_handoffCloseFail=0, g_satelliteBlockedByCore=0;
...
int      DN_hADX = INVALID_HANDLE;      // v3: ADX handle for regime filter
int      DN_hAtrH1_24 = INVALID_HANDLE; // v3: H1 24-period ATR for vol filter numerator
int      DN_hAtrD1_10 = INVALID_HANDLE; // v3: D1 10-period ATR for vol filter denominator
datetime DN_lastBarTime = 0;
int      DN_tradesToday = 0;
int      DN_todayYmd = 0;
int      DN_nBuy = 0, DN_nSell = 0, DN_cRiskSkip = 0, DN_cSpread = 0, DN_cSession = 0;
int      DN_cADX = 0, DN_cVol = 0, DN_cBlockHr = 0, DN_cBody = 0;   // v3: new-filter skip counters
int      DN_nTrailExit = 0, DN_nOppExit = 0;                        // v3: exit-reason counters
bool     DN_blockHr[24];                                            // v3: parsed DN_BlockHours

//====================== MR_Z state (MR_) v3 4th sleeve ==============
int      MR_hATR = INVALID_HANDLE;
int      MR_hMA  = INVALID_HANDLE;
int      MR_hStd = INVALID_HANDLE;
int      MR_hADX = INVALID_HANDLE;      // shares regime data with DONCHIAN but on MR_TF (so kept separate)
datetime MR_lastBarTime = 0;
int      MR_tradesToday = 0;
int      MR_todayYmd = 0;
int      MR_nBuy = 0, MR_nSell = 0, MR_cRiskSkip = 0, MR_cSpread = 0, MR_cSession = 0;
int      MR_cRegime = 0, MR_cZ = 0, MR_cSL = 0;                     // filter skip counters
...
void F_Tick(){
   ...
   if((closeH1>emaH) && F_RecentBreakUp() && (lo1<=emaL) && (c1>o1)&&(c1>emaL)&&(c1>h2)){
      ...
      if(risk>0 && risk<=FIX_MaxSL_ATR*atr){
         ...
         if(G_OpenSlotBlocked(FIX_Magic)){ F_cReject++; return; }
...
void D_OpenLong(){
   ...
   if(G_OpenSlotBlocked(DT_Magic)){ D_cRiskSkip++; return; }   // v3.1: confirmed core entry hands off any satellite first
...
//===================================================================
//  DONCHIAN functions (v3: 3rd sleeve with filters)
//===================================================================
double DN_ATR(){ double b[]; ArraySetAsSeries(b,true); if(CopyBuffer(DN_hAtr,0,0,1,b)<=0) return(0); return(b[0]); }
double DN_ADX(){ double b[]; if(DN_hADX==INVALID_HANDLE) return(0); if(CopyBuffer(DN_hADX,0,1,1,b)<=0) return(0); return(b[0]); }   // v3: ADX(1) from last closed bar
...
ulong DN_GetMyTicket(){ for(int i=PositionsTotal()-1;i>=0;i--){ ulong tk=PositionGetTicket(i); if(tk==0)continue; if(PositionGetInteger(POSITION_MAGIC)==DN_Magic && PositionGetString(POSITION_SYMBOL)==_Symbol) return(tk);} return(0); }
...
// v3: parse DN_BlockHours string once at init, then O(1) lookup
void DN_ParseBlockHours(){
   for(int i=0;i<24;i++) DN_blockHr[i]=false;
   string parts[]; int n=StringSplit(DN_BlockHours,',',parts);
   for(int i=0;i<n;i++){ string s=parts[i]; StringTrimLeft(s); StringTrimRight(s); if(StringLen(s)==0) continue; int h=(int)StringToInteger(s); if(h>=0 && h<24) DN_blockHr[h]=true; }
}
bool DN_BlockedHour(){ MqlDateTime mt; TimeToStruct(TimeCurrent(),mt); return(DN_blockHr[mt.hour]); }
// v3: volatility filter - H1_ATR(24) / D1_ATR(10). >2.0 = unusually volatile day, skip breakout.
bool DN_VolFilterPasses(){
   if(!DN_UseVolFilter) return(true);
   if(DN_hAtrH1_24==INVALID_HANDLE || DN_hAtrD1_10==INVALID_HANDLE) return(true);
   double bH1[1], bD1[1];
   if(CopyBuffer(DN_hAtrH1_24,0,1,1,bH1)<1) return(true);
   if(CopyBuffer(DN_hAtrD1_10,0,1,1,bD1)<1) return(true);
   if(bD1[0]<=0) return(true);
   return((bH1[0]/bD1[0]) <= DN_MaxVolMult);
}

// Returns 1 = LONG signal, -1 = SHORT signal, 0 = none. Evaluates on M15 bar close (bar 1).
// v3: added body-confirmation - close in direction of breakout.
int DN_CheckBreakout(){
   ...
   if(c1>ref_hi){
      if(DN_RequireBody && c1<=o1){ DN_cBody++; return(0); }   // v3: BUY breakout needs bullish close
      return(1);
   }
   if(c1<ref_lo){
      if(DN_RequireBody && c1>=o1){ DN_cBody++; return(0); }   // v3: SELL breakout needs bearish close
      return(-1);
   }
   return(0);
}

// v3: manage open DONCHIAN position - ATR trailing stop + opposite Donchian(DN_ExitLookback) exit
void DN_ManageOpen(){
   if(!DN_UseTrailStop && !DN_UseOppExit) return;
   ulong tk=DN_GetMyTicket(); if(tk==0) return;
   ...
   double c1=iClose(_Symbol,PERIOD_M15,1);
   // opposite Donchian(DN_ExitLookback) reference (from last closed bars, shift 2..N+1)
   double exitHi=-DBL_MAX, exitLo=DBL_MAX;
   if(DN_UseOppExit){
      int lb=DN_ExitLookback;
      for(int i=2; i<=lb+1; i++){
         double h=iHigh(_Symbol,PERIOD_M15,i), l=iLow(_Symbol,PERIOD_M15,i);
         if(h>exitHi) exitHi=h; if(l<exitLo) exitLo=l;
      }
   }
   if(type==POSITION_TYPE_BUY){
      // opposite exit: close below Donchian(DN_ExitLookback) low
      if(DN_UseOppExit && exitLo<DBL_MAX && c1<exitLo){ if(dn.PositionClose(tk)){ DN_nOppExit++; } return; }
      // trailing stop
      if(DN_UseTrailStop){
         double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
         double newSL=NormalizeDouble(bid-DN_TrailATR*atr,dg);
         if(newSL>curSL && (bid-newSL)>=stopsLvl && newSL<bid){
            if(dn.PositionModify(tk,newSL,tp)) DN_nTrailExit++;   // count trail-ups (not exits)
         }
      }
   } else if(type==POSITION_TYPE_SELL){
      // opposite exit: close above Donchian(DN_ExitLookback) high
      if(DN_UseOppExit && exitHi>-DBL_MAX && c1>exitHi){ if(dn.PositionClose(tk)){ DN_nOppExit++; } return; }
      // trailing stop
      if(DN_UseTrailStop){
         double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
         double newSL=NormalizeDouble(ask+DN_TrailATR*atr,dg);
         if((curSL==0 || newSL<curSL) && (newSL-ask)>=stopsLvl && newSL>ask){
            if(dn.PositionModify(tk,newSL,tp)) DN_nTrailExit++;
         }
      }
   }
}
...
void DN_OpenTrade(int direction){
   ...
   if(G_OpenSlotBlocked(DN_Magic)){ DN_cRiskSkip++; return; }   // v3.1: satellite cannot overlap core
   ...
   // v3 NEW: DONCHIAN-specific overnight block (00,01,02 by default - the 6/10 big-loss window)
   if(DN_BlockedHour()){ DN_cBlockHr++; return; }
   // v3 NEW: ADX regime filter - only breakout in trending market
   if(DN_UseADXFilter){ double adx=DN_ADX(); if(adx<DN_MinADX){ DN_cADX++; return; } }
   // v3 NEW: volatility filter - skip during vol spikes (H1_ATR24/D1_ATR10 > DN_MaxVolMult)
   if(!DN_VolFilterPasses()){ DN_cVol++; return; }
   long curPts=(long)SymbolInfoInteger(_Symbol,SYMBOL_SPREAD);
   ...
   // v3: when trailing stop enabled, use TP=0 (unlimited) - trail will manage exit
   if(direction>0){ entry_px=ask; sl_px=entry_px-sl_dist; tp_px = DN_UseTrailStop? 0.0 : entry_px+DN_TP_Rmultiple*sl_dist; }
   else            { entry_px=bid; sl_px=entry_px+sl_dist; tp_px = DN_UseTrailStop? 0.0 : entry_px-DN_TP_Rmultiple*sl_dist; }
   ...
   if(tp_px>0) tp_px=NormalizeDouble(tp_px,dg);
...
void DN_Tick(){
   // v3: manage open positions first (trailing stop + opposite exit on every tick)
   DN_ManageOpen();
...
//===================================================================
//  MR_Z functions (v3 4th sleeve - z-score mean reversion in ranging market)
//===================================================================
double MR_ATR(){ double b[]; if(MR_hATR==INVALID_HANDLE) return(0); if(CopyBuffer(MR_hATR,0,1,1,b)<=0) return(0); return(b[0]); }
double MR_MAv(int s){ double b[]; if(MR_hMA==INVALID_HANDLE) return(0); if(CopyBuffer(MR_hMA,0,s,1,b)<=0) return(0); return(b[0]); }
double MR_STDv(int s){ double b[]; if(MR_hStd==INVALID_HANDLE) return(0); if(CopyBuffer(MR_hStd,0,s,1,b)<=0) return(0); return(b[0]); }
double MR_ADXv(){ double b[]; if(MR_hADX==INVALID_HANDLE) return(0); if(CopyBuffer(MR_hADX,0,1,1,b)<=0) return(0); return(b[0]); }
bool MR_IsNewBar(){ datetime t=iTime(_Symbol,MR_TF,0); if(t!=MR_lastBarTime){ MR_lastBarTime=t; return(true);} return(false); }
int MR_MyPositions(){ int c=0; for(int i=PositionsTotal()-1;i>=0;i--){ ulong tk=PositionGetTicket(i); if(tk==0)continue; if(PositionGetInteger(POSITION_MAGIC)==MR_Magic && PositionGetString(POSITION_SYMBOL)==_Symbol) c++; } return(c); }

bool MR_WithinSession(){
   MqlDateTime mt; TimeToStruct(TimeCurrent(),mt);
   if(mt.hour < MR_SessionStartHour) return(false);
   if(mt.hour > MR_SessionEndHour) return(false);
   if(mt.hour == MR_SessionEndHour && mt.min > 0) return(false);
   return(true);
}
bool MR_FridayEveningBlocked(){
   if(!MR_BlockFridayEvening) return(false);
   MqlDateTime mt; TimeToStruct(TimeCurrent(),mt);
   if(mt.day_of_week==5 && mt.hour>=MR_FridayBlockHour) return(true);
   return(false);
}

double MR_FixedLotAdjusted(){
   double lot=MR_FixedLot;
   double mn=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN), mx=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MAX), st=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_STEP);
   if(st>0) lot=MathFloor(lot/st)*st;
   if(lot<mn) lot=mn; if(mx>0 && lot>mx) lot=mx;
   lot=G_ClampLotToMargin(lot);
...
// Signal: bar 1 closed +/- MR_EntryZ standard deviations from SMA20, with bar body confirming reversal direction.
// Returns 1 = BUY (oversold + bullish bar), -1 = SELL (overbought + bearish bar), 0 = no signal.
int MR_CheckSignal(){
   double sma=MR_MAv(1), sd=MR_STDv(1); if(sd<=0 || sma<=0) return(0);
   double c1=iClose(_Symbol,MR_TF,1), o1=iOpen(_Symbol,MR_TF,1);
   double z=(c1-sma)/sd;
   // Oversold: z <= -MR_EntryZ AND bullish bar (bounce started)
   if(z <= -MR_EntryZ && c1>o1) return(1);
   // Overbought: z >= +MR_EntryZ AND bearish bar (rejection started)
   if(z >=  MR_EntryZ && c1<o1) return(-1);
   MR_cZ++;
   return(0);
}

void MR_OpenTrade(int direction){
   if(MR_MyPositions()>0) return;
   if(MR_tradesToday>=MR_MaxTradesPerDay) return;
   if(!MR_WithinSession()){ MR_cSession++; return; }
   if(MR_FridayEveningBlocked()){ MR_cSession++; return; }
   if(G_OpenSlotBlocked(MR_Magic)){ MR_cRiskSkip++; return; }
   if(G_NewsBlocked()){ MR_cRiskSkip++; return; }
   if(G_EntryHourBlocked()){ MR_cSession++; return; }
   long curPts=(long)SymbolInfoInteger(_Symbol,SYMBOL_SPREAD);
   long maxPts=(long)MathRound(MR_MaxSpreadPrice/SymbolInfoDouble(_Symbol,SYMBOL_POINT));
   if(curPts>maxPts){ MR_cSpread++; return; }

   // Regime check: ONLY trade MR when ADX < MR_MaxADX (ranging market)
   double adx=MR_ADXv();
   if(adx<=0 || adx>=MR_MaxADX){ MR_cRegime++; return; }

   double atr=MR_ATR(); if(atr<=0) return;
   double sl_dist=MR_SL_ATR*atr;
   if(sl_dist > MR_MaxSL_ATR*atr){ MR_cSL++; return; }   // shouldn't happen since sl_dist=1.5*atr, guard anyway

   double sma=MR_MAv(1); if(sma<=0) return;   // TP = SMA (mean)
   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
   double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
   int    dg =(int)SymbolInfoInteger(_Symbol,SYMBOL_DIGITS);
   ...
   double entry_px, sl_px, tp_px;
   if(direction>0){
      entry_px=ask; sl_px=entry_px-sl_dist; tp_px=sma;
      if(tp_px - entry_px < MR_MinTP_ATR*atr){ MR_cSL++; return; }   // TP too close
   } else {
      entry_px=bid; sl_px=entry_px+sl_dist; tp_px=sma;
      if(entry_px - tp_px < MR_MinTP_ATR*atr){ MR_cSL++; return; }
   }

   double lot=MR_FixedLotAdjusted(); if(lot<=0){ MR_cRiskSkip++; return; }
   if(G_RiskBlocksEntry(sl_dist*G_MPP()*lot)){ MR_cRiskSkip++; return; }

   sl_px=NormalizeDouble(sl_px,dg);
   tp_px=NormalizeDouble(tp_px,dg);

   bool ok=false;
   if(direction>0) ok=mr.Buy(lot,_Symbol,0,sl_px,tp_px,"MR_L");
   else            ok=mr.Sell(lot,_Symbol,0,sl_px,tp_px,"MR_S");

   if(ok){ MR_tradesToday++; if(direction>0) MR_nBuy++; else MR_nSell++; }
}

void MR_Tick(){
   if(!MR_IsNewBar()) return;
   MqlDateTime mt; TimeToStruct(iTime(_Symbol,MR_TF,0),mt);
   int today_ymd=mt.year*10000 + mt.mon*100 + mt.day;
   if(today_ymd != MR_todayYmd){ MR_todayYmd=today_ymd; MR_tradesToday=0; }
   int signal=MR_CheckSignal();
   if(signal==0) return;
   MR_OpenTrade(signal);
}
...
void G_CloseAllBoth(){
   // v3: now includes all FOUR sleeves (FIX09, DTREND, DONCHIAN, MR_Z)
   for(int i=PositionsTotal()-1;i>=0;i--){ ulong tk=PositionGetTicket(i); if(tk==0)continue;
      ...
      else if(mg==MR_Magic){ mr.PositionClose(tk); } }
}
...
bool G_RiskBlocksEntry(double riskMoney){
   ...
   if(g_funded){
      // FUNDED: per-trade cap referenced to INITIAL, matching FundedNext risk-limit basis
...
// v3: total open positions across ALL FOUR sleeves. FN 3% funded rule sums them.
int G_MyOpenTotal(){
   int c=0;
   for(int i=PositionsTotal()-1;i>=0;i--){ ulong tk=PositionGetTicket(i); if(tk==0)continue;
      if(PositionGetString(POSITION_SYMBOL)!=_Symbol)continue; long mg=PositionGetInteger(POSITION_MAGIC);
      if(mg==FIX_Magic || mg==DT_Magic || mg==DN_Magic || mg==MR_Magic) c++; }
   return(c);
}
int G_CoreOpenTotal(){
   int c=0;
...
int G_SatelliteOpenTotal(){
   int c=0;
   for(int i=PositionsTotal()-1;i>=0;i--){ ulong tk=PositionGetTicket(i); if(tk==0)continue;
      if(PositionGetString(POSITION_SYMBOL)!=_Symbol)continue; long mg=PositionGetInteger(POSITION_MAGIC);
      if(mg==DN_Magic || mg==MR_Magic) c++; }
   return(c);
}
bool G_CloseSatellitesForCore(){
   bool allClosed=true;
   for(int i=PositionsTotal()-1;i>=0;i--){
      ulong tk=PositionGetTicket(i); if(tk==0)continue;
      if(PositionGetString(POSITION_SYMBOL)!=_Symbol)continue;
      long mg=PositionGetInteger(POSITION_MAGIC); if(mg!=DN_Magic && mg!=MR_Magic)continue;
      double pnl=PositionGetDouble(POSITION_PROFIT)+PositionGetDouble(POSITION_SWAP);
      bool sent=(mg==DN_Magic)?dn.PositionClose(tk):mr.PositionClose(tk);
      uint rc=(mg==DN_Magic)?dn.ResultRetcode():mr.ResultRetcode();
      bool done=sent && (rc==TRADE_RETCODE_DONE || rc==TRADE_RETCODE_DONE_PARTIAL);
      if(done){ g_handoffClosed++; PrintFormat("[HANDOFF] core signal banked satellite magic=%I64d pnl=%.2f",mg,pnl); }
      else { g_handoffCloseFail++; allClosed=false; PrintFormat("[HANDOFF] satellite close FAILED magic=%I64d ticket=%I64u rc=%u",mg,tk,rc); }
   }
   return(allClosed && G_SatelliteOpenTotal()==0);
}
bool G_OpenSlotBlocked(long incomingMagic){
   bool isCore=(incomingMagic==FIX_Magic || incomingMagic==DT_Magic);
   bool isSatellite=(incomingMagic==DN_Magic || incomingMagic==MR_Magic);
   if(Combo_UseProfitHandoff){
      if(isCore && G_SatelliteOpenTotal()>0){
         if(!G_CloseSatellitesForCore()) return(true);
      }
      if(isSatellite && (G_CoreOpenTotal()>0 || G_SatelliteOpenTotal()>0)){
         g_satelliteBlockedByCore++; return(true);
      }
   }
   if(g_funded){
   ...
   if(Combo_EvalMaxOpenTotal>0) return(G_MyOpenTotal() >= Combo_EvalMaxOpenTotal);
   return(false);
}
...
// v3: combined FLOATING PnL of all FOUR sleeves' open positions - used by FUNDED 3% pre-empt
double G_MyFloatingPnL(){
   double s=0;
   for(int i=PositionsTotal()-1;i>=0;i--){ ulong tk=PositionGetTicket(i); if(tk==0)continue;
      if(PositionGetString(POSITION_SYMBOL)!=_Symbol)continue; long mg=PositionGetInteger(POSITION_MAGIC);
      if(mg!=FIX_Magic && mg!=DT_Magic && mg!=DN_Magic && mg!=MR_Magic)continue; s+=PositionGetDouble(POSITION_PROFIT)+PositionGetDouble(POSITION_SWAP); }
   return(s);
}
...
double G_OpenSLRisk(){
   ...
   for(int i = PositionsTotal()-1; i >= 0; i--){
      ...
      if(mg != FIX_Magic && mg != DT_Magic && mg != DN_Magic && mg != MR_Magic) continue;
...
int OnInit(){
   ...
   mr.SetExpertMagicNumber(MR_Magic);  mr.SetDeviationInPoints(30); mr.SetTypeFillingBySymbol(_Symbol); mr.LogLevel(LOG_LEVEL_NO);
   // FIX09 handles
   ...
   // DONCHIAN handles (M15 ATR for SL/TP; v3 adds ADX + H1/D1 ATR for filters)
   DN_hAtr      = iATR(_Symbol, PERIOD_M15, DN_ATRPeriod);
   DN_hADX      = iADX(_Symbol, PERIOD_M15, DN_ADXPeriod);      // v3: regime filter
   DN_hAtrH1_24 = iATR(_Symbol, PERIOD_H1,  24);                 // v3: vol filter numerator
   DN_hAtrD1_10 = iATR(_Symbol, PERIOD_D1,  10);                 // v3: vol filter denominator
   DN_ParseBlockHours();                                          // v3: parse "0,1,2" -> DN_blockHr[]
   // MR_Z handles (v3 4th sleeve: MA + StdDev + ATR + ADX on MR_TF)
   MR_hATR = iATR(_Symbol,    MR_TF, MR_ATRPeriod);
   MR_hMA  = iMA(_Symbol,     MR_TF, MR_MAPeriod, 0, MODE_SMA, PRICE_CLOSE);
   MR_hStd = iStdDev(_Symbol, MR_TF, MR_MAPeriod, 0, MODE_SMA, PRICE_CLOSE);
   MR_hADX = iADX(_Symbol,    MR_TF, 14);
   if(F_hEmaHTF==INVALID_HANDLE||F_hEmaLTF==INVALID_HANDLE||F_hAtr==INVALID_HANDLE||
      ...
      DN_hAtr==INVALID_HANDLE || DN_hADX==INVALID_HANDLE || DN_hAtrH1_24==INVALID_HANDLE || DN_hAtrD1_10==INVALID_HANDLE ||
      MR_hATR==INVALID_HANDLE || MR_hMA==INVALID_HANDLE || MR_hStd==INVALID_HANDLE || MR_hADX==INVALID_HANDLE)
      return(INIT_FAILED);
   ...
   PrintFormat("[COMBO_v3.1_HANDOFF] init bal=%.2f login=%s MODE=%s  FIX09=%s DTREND=%s DONCHIAN=%s MR_Z=%s  staticDDstop=%.1f%% dailyLoss=%.1f%%",
               g_initBal,IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN)),(g_funded?"FUNDED":"EVAL"),
               (Combo_EnableFIX09?"on":"off"),(Combo_EnableDTREND?"on":"off"),(Combo_EnableDonchian?"on":"off"),(Combo_EnableMR?"on":"off"),
               Combo_StaticDDStopPct,Combo_DailyLossPct);
   PrintFormat("[COMBO_v3] DONCHIAN filters: ADX>=%.1f=%s  volMult<=%.1f=%s  blockHr=%s  body=%s  trail=%s  oppExit=%s",
               DN_MinADX,(DN_UseADXFilter?"ON":"off"),DN_MaxVolMult,(DN_UseVolFilter?"ON":"off"),
               DN_BlockHours,(DN_RequireBody?"ON":"off"),(DN_UseTrailStop?"ON":"off"),(DN_UseOppExit?"ON":"off"));
...
void OnDeinit(const int r){
   ...
   if(DN_hADX!=INVALID_HANDLE) IndicatorRelease(DN_hADX);
   if(DN_hAtrH1_24!=INVALID_HANDLE) IndicatorRelease(DN_hAtrH1_24);
   if(DN_hAtrD1_10!=INVALID_HANDLE) IndicatorRelease(DN_hAtrD1_10);
   if(MR_hATR!=INVALID_HANDLE) IndicatorRelease(MR_hATR);
   if(MR_hMA!=INVALID_HANDLE)  IndicatorRelease(MR_hMA);
   if(MR_hStd!=INVALID_HANDLE) IndicatorRelease(MR_hStd);
   if(MR_hADX!=INVALID_HANDLE) IndicatorRelease(MR_hADX);
}
void OnTick(){
   ...
   if(Combo_EnableMR)       MR_Tick();
}
double OnTester(){
   PrintFormat("[COMBO_v3] DTREND buys=%d exits=%d flatDown=%d chopADX=%d htfSkip=%d stdvTP=%d stdvNoTgt=%d | FIX09 rejects=%d | DONCHIAN buys=%d sells=%d riskSkip=%d spread=%d session=%d adx=%d vol=%d blockHr=%d body=%d trailUp=%d oppExit=%d | MR buys=%d sells=%d regime=%d z=%d sl=%d",
               D_nBuy,D_nExit,D_cFlatDown,D_cChop,D_cHTF,D_nStdvTP,D_cStdvNoTgt,F_cReject,
               DN_nBuy,DN_nSell,DN_cRiskSkip,DN_cSpread,DN_cSession,DN_cADX,DN_cVol,DN_cBlockHr,DN_cBody,DN_nTrailExit,DN_nOppExit,
               MR_nBuy,MR_nSell,MR_cRegime,MR_cZ,MR_cSL);
   int h=FileOpen("ck_gold_combo_v3_trades.csv",FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI,",");
   if(h!=INVALID_HANDLE){ FileWrite(h,"time","profit","magic"); HistorySelect(0,TimeCurrent()); int total=HistoryDealsTotal();
      for(int i=0;i<total;i++){ ulong tk=HistoryDealGetTicket(i); if(tk==0)continue; if(HistoryDealGetString(tk,DEAL_SYMBOL)!=_Symbol)continue; if(HistoryDealGetInteger(tk,DEAL_ENTRY)!=DEAL_ENTRY_OUT)continue;
         long mg=HistoryDealGetInteger(tk,DEAL_MAGIC); if(mg!=FIX_Magic && mg!=DT_Magic && mg!=DN_Magic && mg!=MR_Magic)continue;
   ...
   int h2=FileOpen("ck_gold_combo_v3_deals.csv",FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI,",");
   if(h2!=INVALID_HANDLE){
      ...
      for(int i=0;i<ndl;i++){ ulong o=HistoryDealGetTicket(i); if(o==0)continue;
         ...
         long mg=HistoryDealGetInteger(o,DEAL_MAGIC); if(mg!=FIX_Magic && mg!=DT_Magic && mg!=DN_Magic && mg!=MR_Magic)continue;
