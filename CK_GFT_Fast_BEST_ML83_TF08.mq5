...
//+------------------------------------------------------------------+
#property copyright "CK GFT Fast BEST + ML83 + TF08"
#property version   "25.20"
#property strict
//------------------------------------------------------------------//
//  ADDED on top of the BEST-performing build (net +$10,511 backtest):|
//   1. HARD daily loss cap = $83  (InpMaxDailyLossMoney). If the      |
//      day's loss reaches $83, any open trade is closed and no more   |
//      trades are taken that day. Guarantees daily loss <= $83.       |
//   2. TIME FILTER: no NEW entries during 08:00-08:59 server time     |
//      (worst-loss hour found in the backtest data). Open trades are  |
//      NOT closed by the filter - they run to their own SL/TP.        |
//  Everything else (entries, RR, EMA, partial TP, BE) is UNCHANGED.   |
//------------------------------------------------------------------//
...
//=== HARD DAILY LOSS CAP (user: max $83/day) ===
input double InpMaxDailyLossMoney = 83.0;  // Absolute max loss per day ($); 0 = off

//=== TIME FILTER (block worst-loss hour 08:00) ===
input bool   InpUseTimeFilter    = true;   // Block NEW entries during hour window below
input int    InpBlockHourStart   = 8;      // from this server hour (inclusive, 0-23)
...
int      g_tradesToday= 0;
bool     g_dailyLossHit = false; // hard $83 daily cap reached -> stop for the day
...
bool InBlockedHour()
{
   ...
   return(h >= InpBlockHourStart || h <= InpBlockHourEnd);   // wraps past midnight
}
...
void ManageTrade()
{
   ...
   if(InpUsePartialTP1 && g_partialsDone == 0 && progress >= InpTP1Progress)
   {
   ...
   //=== TP2 at 60% progress (DISABLED in v23) ===
   if(InpUsePartialTP2 && g_partialsDone == 1 && progress >= InpTP2Progress)
   {
...
//+------------------------------------------------------------------+
//| HARD daily loss cap: guarantee day's loss never exceeds $83.      |
//| Uses equity so a floating loss that reaches the cap is closed.    |
//| Returns true if the day is stopped (cap already hit or just hit). |
//+------------------------------------------------------------------+
bool DailyLossGuard()
{
...
bool TradingAllowed()
{
   if(g_dailyLossHit) return(false);                // hard $83 cap reached today
   if(InBlockedHour()) return(false);               // time filter: skip worst-loss hour
...
void OnTick()
{
   ...
   // Hard $83 daily loss cap: closes open trade + stops for the day if hit
