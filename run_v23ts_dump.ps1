...
Write-Host "[1/4] downloading + compiling CK_GFT_v23_ts..."
...
Write-Host "[3/4] backtest v23 - REAL TICKS, M15, risk 0.5%, MaxLot 0.09 (several min)..."
$ini=Join-Path $research "v23ts.ini"
...
Write-Host "[4/4] printing v23 trades (with time) - COPY EVERYTHING BETWEEN THE MARKERS:"
Write-Host ""
Write-Host "=====V23TS_CSV_START====="
if(Test-Path $csv){ Get-Content $csv } else { Write-Host "NO CSV - failed." }
Write-Host "=====V23TS_CSV_END====="
Write-Host ""
Write-Host "^ Copy the whole block and paste it back to me (to combine v23 + AMA)."
