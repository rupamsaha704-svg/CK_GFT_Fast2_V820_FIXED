...
$exp=Join-Path $dataDir "MQL5\Experts"; $common=Join-Path $dataDir "..\Common\Files"; $csv=Join-Path $common "ck_v27_trades.csv"

Write-Host "[1/4] downloading + compiling v27 (v23 + regime position sizing)..."
Invoke-WebRequest "$base/CK_GFT_Fast_v27_ROBUST.mq5" -OutFile (Join-Path $exp "CK_GFT_Fast_v27_ROBUST.mq5")
$mq=Join-Path $exp "CK_GFT_Fast_v27_ROBUST.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
Write-Host "[3/4] backtest - REAL TICKS, base risk 1.0% x regime factor, MaxLot 0.09 (several min)..."
$ini=Join-Path $research "v27.ini"
$cfg="[Tester]`nExpert=CK_GFT_Fast_v27_ROBUST.ex5`nSymbol=XAUUSD`nPeriod=M15`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n[TesterInputs]`nInpRR=3.0`nInpMaxSL_ATR=2.5`nInpBaseRisk=1.0`nInpMaxLot=0.09`nInpADXRef=25.0`nInpMinF=0.4`nInpMaxF=2.0`n"
...
Write-Host ""
Write-Host "=====V27_CSV_START====="
if(Test-Path $csv){ Get-Content $csv } else { Write-Host "NO CSV - failed." }
Write-Host "=====V27_CSV_END====="
Write-Host ""
Write-Host "^ Copy the whole block and paste it back to me (overfit + Monte Carlo + compare to v23)."
