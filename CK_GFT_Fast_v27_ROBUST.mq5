//+------------------------------------------------------------------+
//|                                    CK_GFT_Fast_v27_ROBUST.mq5     |
//|  = v23 trend logic (UNCHANGED entries) + REGIME POSITION SIZING. |
//|                                                                    |
//|  Documented, non-overfit improvement (algotr / prop research):    |
//|  scale risk with trend strength -> bigger size when a real trend  |
//|  exists (ADX high), smaller in chop (ADX low). Entries identical  |
//|  to v23, so this only re-weights position size by regime.         |
//|  risk% = InpBaseRisk * clamp(ADX/InpADXRef, InpMinF, InpMaxF).    |
//|  HARD MaxLot 0.09 cap still applies. Validate OOS via Vibe.        |
//+------------------------------------------------------------------+
#property copyright "CK GFT Robust v27"
...
input double InpBaseRisk         = 1.0;     // base % risk (scaled by regime)
...
input double InpADXRef           = 25.0;    // ADX that maps to 1.0x size
input double InpMinF             = 0.4;     // min size factor (weak trend)
input double InpMaxF             = 2.0;     // max size factor (strong trend)
...
void ResetDaily(){ g_dayStart=iTime(_Symbol,PERIOD_D1,0); g_dayStartBal=AccountInfoDouble(ACCOUNT_BALANCE); g_oneR_money=g_dayStartBal*(InpBaseRisk/100.0); g_tradesToday=0; }
...
double RegimeFactor(){ double adx=ADX(); if(adx<=0)return(1.0); double f=adx/InpADXRef; if(f<InpMinF)f=InpMinF; if(f>InpMaxF)f=InpMaxF; return(f); }
...
void OpenBuy(double sl,double tp){ double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK); double risk=ask-sl; if(risk<=0)return; double rm=AccountInfoDouble(ACCOUNT_BALANCE)*(InpBaseRisk/100.0)*RegimeFactor(); double lots=LotForRisk(rm,risk); if(lots<=0)return; int dg=(int)SymbolInfoInteger(_Symbol,SYMBOL_DIGITS); sl=NormalizeDouble(sl,dg); tp=NormalizeDouble(tp,dg); if(trade.Buy(lots,_Symbol,0,sl,tp)){ g_tradesToday++; g_beActivated=false; } }
void OpenSell(double sl,double tp){ double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID); double risk=sl-bid; if(risk<=0)return; double rm=AccountInfoDouble(ACCOUNT_BALANCE)*(InpBaseRisk/100.0)*RegimeFactor(); double lots=LotForRisk(rm,risk); if(lots<=0)return; int dg=(int)SymbolInfoInteger(_Symbol,SYMBOL_DIGITS); sl=NormalizeDouble(sl,dg); tp=NormalizeDouble(tp,dg); if(trade.Sell(lots,_Symbol,0,sl,tp)){ g_tradesToday++; g_beActivated=false; } }
void ManageTrade(){
...
double OnTester(){
   int h=FileOpen("ck_v27_trades.csv", FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI, ",");
   if(h!=INVALID_HANDLE){
