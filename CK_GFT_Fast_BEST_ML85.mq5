...
//+------------------------------------------------------------------+
#property copyright "CK GFT Fast BEST + ML85"
#property version   "25.30"
#property strict
//------------------------------------------------------------------//
//  On top of the BEST-performing build (net +$10,511 backtest):      |
//   1. ONLY new input: hard daily loss cap = $85 (InpMaxDailyLossMoney)|
//      -> day's loss (incl. floating) never exceeds $85; on hit the   |
//         open trade is closed and no more trades that day.           |
//   2. Partial booking: NOTHING is booked before 50% of TP distance.  |
//      At 50% -> book a FIXED 0.02 lot, the rest (0.06/0.07/...) runs  |
//      to full TP.  (No time filter - removed as requested.)          |
//   Entries, RR, EMA, lot sizing, BE@65% are UNCHANGED.               |
//------------------------------------------------------------------//
...
input bool   InpUsePartialTP1    = false;  // TP1 disabled - nothing booked before 50%
input double InpTP1Progress      = 0.10;
input double InpTP1CloseRatio    = 0.0;
input bool   InpUsePartialTP2    = true;   // book a fixed lot at 50%
input double InpTP2Progress      = 0.50;   // book only after 50% of TP distance
input double InpTP2BookLot        = 0.02;  // FIXED lot to book at 50% (rest runs to TP)

input bool   InpUseBreakEven     = true;
input double InpBEProgress       = 0.65;

//=== HARD DAILY LOSS CAP (user: max $85/day) - ONLY added input ===
input double InpMaxDailyLossMoney = 85.0;  // Absolute max loss per day ($); 0 = off
...
int      g_tradesToday= 0;
bool     g_dailyLossHit = false; // hard $85 daily cap reached -> stop for the day
...
void ResetDaily()
{
   ...
   g_tradesToday = 0;
   g_dailyLossHit= false;   // reset hard daily loss cap each new day
}
...
void ManageTrade()
{
   ...
   //=== PARTIAL BOOK: nothing before 50%. At >=50% of TP distance,   ===
   //=== book a FIXED 0.02 lot, then let the rest run to full TP.      ===
   if(InpUsePartialTP2 && g_partialsDone == 0 && progress >= InpTP2Progress)
   {
      double volToClose = NormalizePartialVolume(InpTP2BookLot);
      if(volToClose > 0 && currentVol > volToClose && (currentVol - volToClose) >= mnLot)
      {
         if(trade.PositionClosePartial(ticket, volToClose))
         {
            g_partialsDone = 2;
            Print(">>> BOOK ", volToClose, " @ ", (int)(progress*100), "% - rest runs to TP");
         }
      }
      else
      {
         g_partialsDone = 2;   // remaining too small to split -> let it run to TP
      }
   }
...
//+------------------------------------------------------------------+
//| HARD daily loss cap: guarantee day's loss never exceeds $85.      |
//| Uses equity so a floating loss reaching the cap is closed too.    |
//+------------------------------------------------------------------+
bool DailyLossGuard()
{
   if(g_dailyLossHit) return(true);
   if(InpMaxDailyLossMoney <= 0) return(false);
   double eq   = AccountInfoDouble(ACCOUNT_EQUITY);
   double loss = g_dayStartBal - eq;               // today's loss (incl. floating)
   if(loss >= InpMaxDailyLossMoney)
   {
      ulong tk = GetMyTicket();
      if(tk > 0) trade.PositionClose(tk);          // close open trade at the cap
      ResetTradeState();
      g_dailyLossHit = true;
      Print(">>> DAILY LOSS CAP ($", InpMaxDailyLossMoney, ") hit - stopped for today");
      return(true);
   }
   return(false);
}
...
bool TradingAllowed()
{
   if(g_dailyLossHit) return(false);                // hard $85 cap reached today
...
void OnTick()
{
   ...
   // Hard $85 daily loss cap: closes open trade + stops for the day if hit
