...
Write-Host "[1/4] downloading + compiling diagnostic EA..."
Invoke-WebRequest "$base/CK_GFT_v23_diag.mq5" -OutFile (Join-Path $exp "CK_GFT_v23_diag.mq5")
$mq=Join-Path $exp "CK_GFT_v23_diag.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
Write-Host "[2/4] clearing old diag CSV..."
$csv=Join-Path $common "ck_v23_diag.csv"; if(Test-Path $csv){Remove-Item $csv -Force -ErrorAction SilentlyContinue}

Write-Host "[3/4] backtest - REAL TICKS, risk 0.5%, MaxLot 0.09 (several min)..."
$ini=Join-Path $research "diag.ini"
$cfg="[Tester]`nExpert=CK_GFT_v23_diag.ex5`nSymbol=XAUUSD`nPeriod=M15`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n[TesterInputs]`nInpRR=3.0`nInpMaxSL_ATR=2.5`nInpRiskPercent=0.5`nInpMaxLot=0.09`n"
...
Write-Host "[4/4] printing per-trade diagnostic CSV - COPY EVERYTHING BETWEEN THE MARKERS:"
Write-Host ""
Write-Host "=====DIAG_CSV_START====="
if(Test-Path $csv){ Get-Content $csv } else { Write-Host "NO CSV - failed." }
Write-Host "=====DIAG_CSV_END====="
Write-Host ""
