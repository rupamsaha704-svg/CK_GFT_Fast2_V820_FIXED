//+------------------------------------------------------------------+
//|  CK_GFT_v23_regime.mq5 — v23_live + ONE objective regime gate.    |
//|  All v23 logic UNCHANGED. Adds an evidence-motivated trend-regime |
//|  filter: only trade when the HTF EMA200 has moved at least        |
//|  InpRegimeMinSlopeATR * HTF-ATR over InpRegimeLookback bars       |
//|  (i.e. a real trend exists). InpUseRegime toggles it so the SAME  |
//|  binary gives a clean OFF(baseline)-vs-ON A/B. Single, non-       |
//|  optimized threshold; must be OOS/CPCV-validated before adoption. |
//+------------------------------------------------------------------+
#property copyright "CK GFT v23 regime"
#property version   "23.31r"
#property strict
...
// --- regime gate (the ONE new thing) ---
input bool   InpUseRegime        = true;   // OFF => identical to v23_live
input int    InpRegimeLookback   = 20;     // HTF bars to measure EMA200 slope over
input double InpRegimeMinSlopeATR= 0.5;    // require |EMA200 move| >= this * HTF-ATR

int      hEmaHTF,hEmaLTF,hAtr,hAtrHTF;
datetime lastBarTime=0,g_dayStart=0;
double   g_dayStartBal=0,g_oneR_money=0;
int      g_tradesToday=0; bool g_beActivated=false;
int      g_cSpread=0,g_cSubMin=0,g_cReject=0,g_cRegime=0;
...
int OnInit(){
   ...
   hAtrHTF=iATR(_Symbol,InpHTF,14);
   if(hEmaHTF==INVALID_HANDLE||hEmaLTF==INVALID_HANDLE||hAtr==INVALID_HANDLE||hAtrHTF==INVALID_HANDLE)return(INIT_FAILED);
...
void OnDeinit(const int r){ if(hEmaHTF!=INVALID_HANDLE)IndicatorRelease(hEmaHTF); if(hEmaLTF!=INVALID_HANDLE)IndicatorRelease(hEmaLTF); if(hAtr!=INVALID_HANDLE)IndicatorRelease(hAtr); if(hAtrHTF!=INVALID_HANDLE)IndicatorRelease(hAtrHTF); }

double ATR(){ double b[]; if(CopyBuffer(hAtr,0,0,1,b)<=0)return(0); return(b[0]); }
double ATRHTF(){ double b[]; if(CopyBuffer(hAtrHTF,0,1,1,b)<=0)return(0); return(b[0]); }
...
// regime gate: is the HTF trend strong enough (EMA200 sloping) to be a "trend regime"?
bool RegimeTrending(){
   if(!InpUseRegime) return(true);
   double now=EmaHTF(1), past=EmaHTF(1+InpRegimeLookback), atrH=ATRHTF();
   if(now<=0||past<=0||atrH<=0) return(true);       // fail-open: never block on missing data
   double slope=MathAbs(now-past);
   return(slope >= InpRegimeMinSlopeATR*atrH);
}
...
bool CountIfFilled(bool sent,string tag){ uint rc=trade.ResultRetcode(); bool filled=sent&&(rc==TRADE_RETCODE_DONE||rc==TRADE_RETCODE_DONE_PARTIAL)&&trade.ResultDeal()!=0; if(filled){ g_tradesToday++; g_beActivated=false; GlobalVariableSet(GK_TRD,g_tradesToday);} else { g_cReject++; } return(filled); }
...
double OnTester(){
   PrintFormat("[v23regime] useRegime=%s regimeBlocked=%d subMinSkips=%d orderRejects=%d",(InpUseRegime?"true":"false"),g_cRegime,g_cSubMin,g_cReject);
   int h=FileOpen("ck_v23regime_trades.csv", FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI, ",");
   if(h!=INVALID_HANDLE){
...
void OnTick(){
   ...
   bool trending=RegimeTrending();                                  // regime gate
