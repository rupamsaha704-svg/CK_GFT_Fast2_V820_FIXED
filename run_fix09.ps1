...
# Compile CK_GOLD_PRO_FIX09 (fixed 0.09 every trade) + backtest to confirm it matches ~+200%.
...
Write-Host "[1/4] downloading + compiling CK_GOLD_PRO_FIX09..."
...
Write-Host "[3/4] backtest fixed 0.09, real ticks, 2025.08-2026.08..."
...
Write-Host ("Return       : {0:N1}%%   (should be ~ +200%% like the 2.0%% capped run)" -f $ret)
Write-Host ("Profit factor: {0:N2}" -f $pf)
Write-Host ("Win rate     : {0:N1}%%" -f $wr)
Write-Host ("Closed-trade max DD: {0:N1}%%" -f ($mdd*100))
Write-Host "=====FIX09_END====="
Write-Host "^ Copy this block back to me. Every trade used exactly 0.09 (check the Journal 'volume' column too)."
