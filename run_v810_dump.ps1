...
$exp=Join-Path $dataDir "MQL5\Experts"; $common=Join-Path $dataDir "..\Common\Files"; $csv=Join-Path $common "ck_v810_trades.csv"

Write-Host "[1/4] downloading + compiling CK_GFT_v810_diag..."
Invoke-WebRequest "$base/CK_GFT_v810_diag.mq5" -OutFile (Join-Path $exp "CK_GFT_v810_diag.mq5")
$mq=Join-Path $exp "CK_GFT_v810_diag.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
Write-Host "[3/4] backtest - REAL TICKS, M15, MaxLot 0.08 (several min)..."
$ini=Join-Path $research "v810.ini"
$cfg="[Tester]`nExpert=CK_GFT_v810_diag.ex5`nSymbol=XAUUSD`nPeriod=M15`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n"
