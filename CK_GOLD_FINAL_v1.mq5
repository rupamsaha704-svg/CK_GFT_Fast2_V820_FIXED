//+------------------------------------------------------------------+
//|                                          CK_GOLD_FINAL_v1.mq5     |
//|  FINAL deployable = identical logic to the validated v23         |
//|  (trend breakout-pullback, XAUUSD M15). Renamed for clean deploy  |
//|  and for the user's own independent MT5 backtest verification.    |
//|                                                                    |
//|  Reproduce the reported result in the Strategy Tester:            |
//|   Symbol XAUUSD, M15, model "Every tick based on real ticks",     |
//|   2025.08.01 -> 2026.08.01, deposit 5000, leverage 1:10.          |
//|   Inputs: RR=3.0, MaxSL_ATR=2.5, MaxLot=0.09,                     |
//|           RiskPercent=2.0 -> ~+200% (aggressive, high DD tail)    |
//|           RiskPercent=0.5 -> ~+115% (safest)                      |
//+------------------------------------------------------------------+
#property copyright "CK GOLD FINAL v1 (= validated v23)"
...
input double InpRiskPercent      = 2.0;    // 2.0 ~200% (aggressive) | 0.5 ~115% (safe)
...
double OnTester(){
   int h=FileOpen("ck_gold_final_trades.csv", FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI, ",");
   if(h!=INVALID_HANDLE){
