...
Write-Host "===== STRATEGY #1 v25 = v23 + DRAWDOWN CIRCUIT-BREAKER (halt at 7.5% below peak) ====="
Write-Host "[1/5] downloading v25 EA + analyzer..."
Invoke-WebRequest "$base/CK_GFT_Fast_v25_ROBUST.mq5" -OutFile (Join-Path $exp "CK_GFT_Fast_v25_ROBUST.mq5")
...
$mq=Join-Path $exp "CK_GFT_Fast_v25_ROBUST.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
$csv=Join-Path $common "ck_v25_trades.csv"; if(Test-Path $csv){Remove-Item $csv -Force -ErrorAction SilentlyContinue}

Write-Host "[4/5] FULL 12-month backtest - REAL TICKS, MaxLot 0.09, risk 0.5%, DD-breaker 7.5% (several min)..."
$ini=Join-Path $research "v25.ini"
$cfg="[Tester]`nExpert=CK_GFT_Fast_v25_ROBUST.ex5`nSymbol=XAUUSD`nPeriod=M15`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n[TesterInputs]`nInpRR=3.0`nInpMaxSL_ATR=2.5`nInpRiskPercent=0.5`nInpMaxLot=0.09`nInpUseDDBreaker=true`nInpMaxDDPct=7.5`n"
...
Write-Host "############ STRATEGY #1 v25 (DD circuit-breaker) ############"
Write-Host "  (v23 NO breaker was: Net +6712  PF 1.46  Win 25.8%  MaxDD 13.1%)"
Write-Host "  GOAL: realized MaxDD <= 9% (breaker enforces it) AND still net positive"
$icsv=Join-Path $common "ck_v25_trades.csv"
if(-not (Test-Path $icsv)){ Write-Host "  NO TRADES FILE - tester failed or no trades."; exit }
& $py (Join-Path $val "analyze_trades.py") $icsv
Write-Host ""
Write-Host "If realized MaxDD <= 9% AND net positive -> STRATEGY #1 DONE. Then start #2."
