//+------------------------------------------------------------------+
//|  CK_GFT_v23_tfilter.mq5 — v23_live + configurable time/day block. |
//|  All v23 logic UNCHANGED. Optionally skips ENTRIES whose bar hour |
//|  (server time) is in InpBlockHours, or whose weekday is in        |
//|  InpBlockDOW (1=Mon..5=Fri). Toggle InpUseTimeFilter for a clean  |
//|  OFF/ON A/B in the SAME binary. WARNING: blocking times chosen    |
//|  from the same sample is overfitting — must be checked on OTHER   |
//|  years before trusting.                                           |
//+------------------------------------------------------------------+
#property copyright "CK GFT v23 tfilter"
#property version   "23.31t"
#property strict
...
// --- time/day filter (the new, toggleable thing) ---
input bool   InpUseTimeFilter    = true;
input string InpBlockHours       = "4,6,13,15";   // server-time hours of the ENTRY bar to skip
input string InpBlockDOW         = "2,4";         // 1=Mon..5=Fri ; e.g. 2=Tue,4=Thu
...
int      g_cSpread=0,g_cSubMin=0,g_cReject=0,g_cTime=0;
string   GK_DAY, GK_BAL, GK_TRD;
int      g_bh[]; int g_bd[];

void ParseInts(string s,int &arr[]){ ArrayResize(arr,0); string parts[]; int k=StringSplit(s,',',parts);
   for(int i=0;i<k;i++){ string t=parts[i]; StringTrimLeft(t); StringTrimRight(t); if(StringLen(t)==0)continue; int n=ArraySize(arr); ArrayResize(arr,n+1); arr[n]=(int)StringToInteger(t); } }
bool InArr(int v,int &arr[]){ for(int i=0;i<ArraySize(arr);i++) if(arr[i]==v) return(true); return(false); }
...
int OnInit(){
   ...
   ParseInts(InpBlockHours,g_bh); ParseInts(InpBlockDOW,g_bd);
   LoadOrResetDaily();
...
bool TimeBlocked(){
   ...
   MqlDateTime dt; TimeToStruct(iTime(_Symbol,PERIOD_CURRENT,0),dt);
   if(InArr(dt.hour,g_bh)) return(true);
   int dow=dt.day_of_week;                 // 0=Sun..6=Sat ; our list uses 1=Mon..5=Fri
   if(InArr(dow,g_bd)) return(true);
   return(false);
}
...
double OnTester(){
   PrintFormat("[v23tf] useFilter=%s timeBlocked=%d subMinSkips=%d orderRejects=%d",(InpUseTimeFilter?"true":"false"),g_cTime,g_cSubMin,g_cReject);
   int h=FileOpen("ck_v23tf_trades.csv", FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI, ",");
   if(h!=INVALID_HANDLE){ FileWrite(h,"time","profit"); HistorySelect(0,TimeCurrent()); int total=HistoryDealsTotal();
      for(int i=0;i<total;i++){ ulong tk=HistoryDealGetTicket(i); if(tk==0)continue;
         if(HistoryDealGetString(tk,DEAL_SYMBOL)!=_Symbol)continue; if(HistoryDealGetInteger(tk,DEAL_ENTRY)!=DEAL_ENTRY_OUT)continue;
...
void OnTick(){
   ...
   if(TimeBlocked()){ g_cTime++; return; }                          // time/day filter
