//+------------------------------------------------------------------+
//|  CK_GOLD_PRO_FIX09_HRS.mq5 - FIX09 + optional entry-hour filter.  |
//|  IDENTICAL trend logic + fixed 0.09 lot to CK_GOLD_PRO_FIX09.     |
//|  ADDS: (1) an ENTRY-time server-hour block filter (block new      |
//|  entries in [BlockStart,BlockEnd) ) to test dropping a losing     |
//|  session; (2) OnTester logs each trade's ENTRY time (not exit)    |
//|  so hour-of-day analysis matches the entry decision.             |
//|  Everything else is byte-for-byte the same behaviour.            |
//+------------------------------------------------------------------+
#property copyright "CK GOLD PRO FIX09 HRS"
...
input bool   InpUseHourFilter    = false;  // block NEW entries during a losing server-hour block
input int    InpBlockStartHour   = 13;     // block [start,end) server hours by ENTRY time
input int    InpBlockEndHour     = 17;
...
int      g_cSpread=0, g_cSpreadDiv=0, g_cSubMin=0, g_cReject=0, g_cHourBlock=0;
...
int OnInit(){
   ...
   GK_DAY="ckgpfh_day_"+scope; GK_BAL="ckgpfh_bal_"+scope; GK_TRD="ckgpfh_trd_"+scope;
   LoadOrResetDaily();
   PrintFormat("[FIX09_HRS] FIXED lot=%.2f  hourFilter=%s block[%d,%d)",
      FixedLot(), (InpUseHourFilter?"ON":"OFF"), InpBlockStartHour, InpBlockEndHour);
...
// True if new entries are blocked right now by the entry-hour filter.
bool HourBlocked(){
   if(!InpUseHourFilter) return(false);
   MqlDateTime dt; TimeToStruct(TimeCurrent(),dt); int h=dt.hour;
   bool blk = (InpBlockStartHour<=InpBlockEndHour) ? (h>=InpBlockStartHour && h<InpBlockEndHour)
                                                   : (h>=InpBlockStartHour || h<InpBlockEndHour);
   return(blk);
}
...
void CountIfFilled(bool sent,string tag){
   ...
   if(filled){ g_tradesToday++; g_beActivated=false; GlobalVariableSet(GK_TRD,g_tradesToday); }
      ...
      PrintFormat("[FIX09_HRS] ORDER_FAIL tag=%s sent=%s rc=%u %s",tag,(sent?"true":"false"),rc,trade.ResultRetcodeDescription()); }
}
...
double OnTester(){
   PrintFormat("[FIX09_HRS] fixedLot=%.2f hourFilter=%s hourBlocks=%d spreadFiltered=%d orderRejects=%d",
      FixedLot(),(InpUseHourFilter?"ON":"OFF"),g_cHourBlock,g_cSpread,g_cReject);
   HistorySelect(0,TimeCurrent());
   int total=HistoryDealsTotal();
   // map position id -> ENTRY time (so we log entry-hour, matching the entry decision)
   ...
   int h=FileOpen("ck_fix09hrs_trades.csv", FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI, ",");
   if(h!=INVALID_HANDLE){
      ...
      for(int i=0;i<total;i++){ ulong tk=HistoryDealGetTicket(i); if(tk==0)continue;
         ...
         datetime et=(datetime)HistoryDealGetInteger(tk,DEAL_TIME);   // fallback: exit time
...
void OnTick(){
   ...
   if(!MarketOpen()) return;
   if(HourBlocked()){ g_cHourBlock++; return; }   // entry-hour filter (new entries only; open trades still managed)
