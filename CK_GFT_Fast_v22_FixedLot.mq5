//+------------------------------------------------------------------+
//|                                    CK_GFT_Fast_v22_FixedLot.mq5  |
//|  Base: v22 (identical logic & optimized inputs)                   |
//|  ONLY CHANGE: lot is FIXED at 0.02 (InpUseFixedLot/InpFixedLot).  |
//|  Risk-% sizing is bypassed; everything else is unchanged.         |
...
//+------------------------------------------------------------------+
#property copyright "CK GFT Fast v22 FixedLot"
...
//=== FIXED LOT (user request: lot fixed at 0.02) ===
input bool   InpUseFixedLot      = true;   // Use a fixed lot (ignore risk % sizing)
...
void OpenBuy()
{
   ...
   double lots;
   if(InpUseFixedLot)
      lots = NormalizePartialVolume(InpFixedLot);   // fixed 0.02 (normalized to broker step)
   else
   {
      double riskMoney = AccountInfoDouble(ACCOUNT_BALANCE) * (InpRiskPercent/100.0);
      lots = LotForRisk(riskMoney, oneR);
   }
   if(lots <= 0) return;
