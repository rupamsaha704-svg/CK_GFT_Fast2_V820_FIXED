...
Write-Host "[1/4] compiling + INSTALLING v23 into MT5 (deploy config: risk 2.0%, MaxLot 0.09)..."
...
Write-Host ""
Write-Host "=====R2_TRADES_CSV_START====="
if(Test-Path $csv){ Get-Content $csv } else { Write-Host "NO CSV - failed." }
Write-Host "=====R2_TRADES_CSV_END====="
Write-Host ""
Write-Host "EA is INSTALLED. In MT5: attach 'CK_GFT_Fast_v23_ROBUST' to XAUUSD M15, set RiskPercent=2.0, MaxLot=0.09."
Write-Host "Copy the CSV block above and paste it back to me for the overfit + Monte Carlo test."
