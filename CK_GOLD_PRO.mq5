//+------------------------------------------------------------------+
//|  CK_GOLD_PRO.mq5 — XAUUSD M15 trend EA (deploy build).            |
//|  Identical validated logic to CK_GFT_v23_live (execution-hardened |
//|  v23): HTF EMA200 trend, breakout, EMA20 pullback, SL from swing  |
//|  +ATR buffer, MaxSL_ATR 2.5, RR 3.0, MaxLot 0.09, BE@+1.5R,       |
//|  ATR shift-0, 3 trades/day, daily +/-R limits. 6 live-safety      |
//|  fixes: price-based spread, verified fill, broker filling mode,   |
//|  sub-min lot skip, persisted daily state, magic-scoped daily R.   |
//|  Default risk 2.0% (backtest 2025.08-2026.08, real ticks:         |
//|  +200%, PF 1.47, closed-DD 16.3%). NOTE: that is a favourable-year |
//|  backtest, NOT a guarantee - demo-first. Set risk 1.7% or 0.5%    |
//|  for a more conservative daily-loss allowance.                    |
//+------------------------------------------------------------------+
#property copyright "CK GOLD PRO"
...
input double InpRiskPercent      = 2.0;    // deploy config; use 1.7 or 0.5 for lower daily-loss allowance
...
input double InpMaxSpreadPrice   = 0.60;   // absolute $ spread (digit-independent)
...
int OnInit(){
   ...
   GK_DAY="ckgp_day_"+scope; GK_BAL="ckgp_bal_"+scope; GK_TRD="ckgp_trd_"+scope;
   LoadOrResetDaily();
   PrintFormat("[CK_GOLD_PRO] digits=%d point=%.5f maxSpreadPts=%d risk=%.2f%%",
...
void LoadOrResetDaily(){
   ...
   if(gday==today && today>0){
      g_dayStart=today; g_dayStartBal=GlobalVariableGet(GK_BAL); g_tradesToday=(int)GlobalVariableGet(GK_TRD);
   } else {
...
void CountIfFilled(bool sent,string tag){
   ...
   if(filled){ g_tradesToday++; g_beActivated=false; GlobalVariableSet(GK_TRD,g_tradesToday); }
      ...
      PrintFormat("[CK_GOLD_PRO] ORDER_FAIL tag=%s sent=%s rc=%u %s deal=%I64u order=%I64u",
...
double OnTester(){
   PrintFormat("[CK_GOLD_PRO] spreadFiltered=%d spreadDivergence=%d subMinSkips=%d orderRejects=%d",g_cSpread,g_cSpreadDiv,g_cSubMin,g_cReject);
   int h=FileOpen("ck_gold_pro_trades.csv", FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI, ",");
   if(h!=INVALID_HANDLE){
