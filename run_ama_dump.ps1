...
$exp=Join-Path $dataDir "MQL5\Experts"; $common=Join-Path $dataDir "..\Common\Files"; $csv=Join-Path $common "ck_ama_trades.csv"

Write-Host "[1/4] downloading + compiling CK_AMA_v1..."
Invoke-WebRequest "$base/CK_AMA_v1.mq5" -OutFile (Join-Path $exp "CK_AMA_v1.mq5")
$mq=Join-Path $exp "CK_AMA_v1.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
Write-Host "[3/4] backtest - REAL TICKS, M15, AMA trend, MaxLot 0.09 (several min)..."
$ini=Join-Path $research "ama.ini"
$cfg="[Tester]`nExpert=CK_AMA_v1.ex5`nSymbol=XAUUSD`nPeriod=M15`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n[TesterInputs]`nInpRR=2.5`nInpRiskPercent=0.5`nInpMaxLot=0.09`nInpLength=14`nInpFast=2`nInpSlow=30`n"
