//+------------------------------------------------------------------+
//|  CK_GOLD_PRO_FIX10_regime.mq5 — FIX09 (fixed 0.09) + PRE-REGISTERED |
//|  leakage-free regime gate (SPEC/regime_classifier_v1: H1 EMA200    |
//|  slope over N=20 bars >= K=0.5 * H1-ATR => TREND). InpUseRegime     |
//|  toggles it for a clean OFF(=FIX09) vs ON A/B. NO other logic change|
//|  vs FIX09. Params are the pre-registered fixed values (not tuned).  |
//+------------------------------------------------------------------+
#property copyright "CK GOLD PRO FIX10 regime"
...
// pre-registered regime gate (frozen values)
input bool   InpUseRegime        = true;
input int    InpRegimeLookback   = 20;
input double InpRegimeMinSlopeATR= 0.5;
...
int OnInit(){
   ...
   hAtr=iATR(_Symbol,PERIOD_CURRENT,14); hAtrHTF=iATR(_Symbol,InpHTF,14);
   ...
   GK_DAY="ckf10_day_"+scope; GK_BAL="ckf10_bal_"+scope; GK_TRD="ckf10_trd_"+scope;
   LoadOrResetDaily();
   PrintFormat("[FIX10regime] useRegime=%s K=%.2f N=%d fixedLot=%.2f",(InpUseRegime?"true":"false"),InpRegimeMinSlopeATR,InpRegimeLookback,FixedLot());
...
double ATRHTF(int s){ double b[]; if(CopyBuffer(hAtrHTF,0,s,1,b)<=0)return(0); return(b[0]); }
...
// PRE-REGISTERED regime gate (causal: uses completed bars only)
bool RegimeTrending(){
   if(!InpUseRegime) return(true);
   double now=EmaHTF(1), past=EmaHTF(1+InpRegimeLookback), atrH=ATRHTF(1);
   if(now<=0||past<=0||atrH<=0) return(true);           // fail-open on missing data
   return(MathAbs(now-past) >= InpRegimeMinSlopeATR*atrH);
}
...
void CountIfFilled(bool sent,string tag){ uint rc=trade.ResultRetcode(); bool filled=sent&&(rc==TRADE_RETCODE_DONE||rc==TRADE_RETCODE_DONE_PARTIAL)&&trade.ResultDeal()!=0; if(filled){ g_tradesToday++; g_beActivated=false; GlobalVariableSet(GK_TRD,g_tradesToday);} else g_cReject++; }
...
double OnTester(){
   PrintFormat("[FIX10regime] useRegime=%s regimeBlocked=%d orderRejects=%d",(InpUseRegime?"true":"false"),g_cRegime,g_cReject);
   int h=FileOpen("ck_fix10_regime_trades.csv", FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI, ",");
