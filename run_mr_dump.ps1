...
Write-Host "[1/4] downloading + compiling MEAN-REVERSION EA (strategy #2)..."
Invoke-WebRequest "$base/CK_MR_StdDev_v1.mq5" -OutFile (Join-Path $exp "CK_MR_StdDev_v1.mq5")
$mq=Join-Path $exp "CK_MR_StdDev_v1.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
$csv=Join-Path $common "ck_mr_trades.csv"; if(Test-Path $csv){Remove-Item $csv -Force -ErrorAction SilentlyContinue}

Write-Host "[3/4] backtest - REAL TICKS, MaxLot 0.09, risk 0.5% (several min)..."
$ini=Join-Path $research "mr_dump.ini"
$cfg="[Tester]`nExpert=CK_MR_StdDev_v1.ex5`nSymbol=XAUUSD`nPeriod=M15`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n[TesterInputs]`nInpRiskPercent=0.5`nInpMaxLot=0.09`nInpMAPeriod=20`nInpEntryZ=2.0`nInpEntrySL_ATR=1.5`n"
...
Write-Host "[4/4] printing trades CSV - COPY EVERYTHING BETWEEN THE MARKERS:"
Write-Host ""
Write-Host "=====MR_TRADES_CSV_START====="
if(Test-Path $csv){ Get-Content $csv } else { Write-Host "NO CSV - EA took 0 trades (z-threshold too strict) or tester failed." }
Write-Host "=====MR_TRADES_CSV_END====="
Write-Host ""
Write-Host "^ Copy the whole block (with START/END markers) and paste it back to me."
