...
//+------------------------------------------------------------------+
#property copyright "CK GFT Fast v26"
...
input double InpMaxLossPerTrade  = 230.0;  // Force-close a trade if its loss exceeds this ($). 0 = off
...
bool MaxLossGuard()
{
   ...
   if(pl <= -InpMaxLossPerTrade)
   {
      ...
      ResetTradeState();
      Print(">>> MAX LOSS GUARD: closed trade at ", pl, " (cap ", InpMaxLossPerTrade, ")");
      return(true);
   }
   return(false);
}
...
void OnTick()
{
   ...
   // v26: check max-loss-per-trade FIRST, every tick
