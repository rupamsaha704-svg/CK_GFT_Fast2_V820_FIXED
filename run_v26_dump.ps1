...
Write-Host "[1/4] downloading + compiling v26 (v23 + ADX regime filter)..."
Invoke-WebRequest "$base/CK_GFT_Fast_v26_ROBUST.mq5" -OutFile (Join-Path $exp "CK_GFT_Fast_v26_ROBUST.mq5")
$mq=Join-Path $exp "CK_GFT_Fast_v26_ROBUST.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
$csv=Join-Path $common "ck_v26_trades.csv"; if(Test-Path $csv){Remove-Item $csv -Force -ErrorAction SilentlyContinue}

Write-Host "[3/4] backtest - REAL TICKS, MaxLot 0.09, risk 0.5%, ADX>=25 (several min)..."
$ini=Join-Path $research "v26_dump.ini"
$cfg="[Tester]`nExpert=CK_GFT_Fast_v26_ROBUST.ex5`nSymbol=XAUUSD`nPeriod=M15`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n[TesterInputs]`nInpRR=3.0`nInpMaxSL_ATR=2.5`nInpRiskPercent=0.5`nInpMaxLot=0.09`nInpMinADX=25.0`n"
...
Write-Host ""
Write-Host "=====V26_TRADES_CSV_START====="
if(Test-Path $csv){ Get-Content $csv } else { Write-Host "NO CSV - 0 trades (ADX filter too strict) or tester failed." }
Write-Host "=====V26_TRADES_CSV_END====="
Write-Host ""
