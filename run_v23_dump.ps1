...
Write-Host "[1/4] downloading + compiling v23 EA..."
...
Write-Host "[3/4] v23 backtest - REAL TICKS, MaxLot 0.09, risk 0.5% (several min)..."
$ini=Join-Path $research "v23_dump.ini"
...
Write-Host "[4/4] printing trades CSV - COPY EVERYTHING BETWEEN THE MARKERS BELOW:"
Write-Host ""
Write-Host "=====V23_TRADES_CSV_START====="
if(Test-Path $csv){ Get-Content $csv } else { Write-Host "NO CSV - backtest produced no trades or failed." }
Write-Host "=====V23_TRADES_CSV_END====="
Write-Host ""
Write-Host "^ Copy the whole block (including START/END markers) and paste it back to me."
