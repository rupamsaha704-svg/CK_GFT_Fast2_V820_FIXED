...
$exp=Join-Path $dataDir "MQL5\Experts"; $common=Join-Path $dataDir "..\Common\Files"; $csv=Join-Path $common "ck_best_trades.csv"

Write-Host "[1/4] downloading + compiling CK_BEST_diag..."
Invoke-WebRequest "$base/CK_BEST_diag.mq5" -OutFile (Join-Path $exp "CK_BEST_diag.mq5")
$mq=Join-Path $exp "CK_BEST_diag.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
Write-Host "[3/4] backtest - REAL TICKS, M5, MaxLot 0.08 (several min)..."
$ini=Join-Path $research "best.ini"
$cfg="[Tester]`nExpert=CK_BEST_diag.ex5`nSymbol=XAUUSD`nPeriod=M5`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n"
...
Write-Host ""
Write-Host "=====BEST_CSV_START====="
if(Test-Path $csv){ Get-Content $csv } else { Write-Host "NO CSV - 0 trades or failed." }
Write-Host "=====BEST_CSV_END====="
Write-Host ""
Write-Host "^ Copy the whole block and paste it back to me (for Vibe validation + combine test with v23)."
