...
$exp=Join-Path $dataDir "MQL5\Experts"; $common=Join-Path $dataDir "..\Common\Files"; $csv=Join-Path $common "ck_ama2_trades.csv"

Write-Host "[1/4] downloading + compiling CK_AMA2_v1 (dual AMA 39/79 + retest + MACD)..."
Invoke-WebRequest "$base/CK_AMA2_v1.mq5" -OutFile (Join-Path $exp "CK_AMA2_v1.mq5")
$mq=Join-Path $exp "CK_AMA2_v1.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
Write-Host "[3/4] backtest - REAL TICKS, M15, dual AMA, MaxLot 0.09 (several min)..."
$ini=Join-Path $research "ama2.ini"
$cfg="[Tester]`nExpert=CK_AMA2_v1.ex5`nSymbol=XAUUSD`nPeriod=M15`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n[TesterInputs]`nInpRR=2.5`nInpRiskPercent=0.5`nInpMaxLot=0.09`nInpFastLen=39`nInpSlowLen=79`nInpUseMACD=true`n"
...
Write-Host ""
Write-Host "=====AMA2_CSV_START====="
if(Test-Path $csv){ Get-Content $csv } else { Write-Host "NO CSV - 0 trades or failed." }
Write-Host "=====AMA2_CSV_END====="
Write-Host ""
Write-Host "^ Copy the whole block and paste it back to me."
