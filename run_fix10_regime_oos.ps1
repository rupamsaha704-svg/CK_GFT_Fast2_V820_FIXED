...
# Test the PRE-REGISTERED regime gate on OOS 2022-25: regime OFF (=FIX09) vs ON, fixed 0.09, ALL params pinned.
# Does gating improve OOS expectancy / cut DD? (SPEC §4). Computes metrics locally (your Python).
...
$py=$null; foreach($c in @("C:\Python314\python.exe","$env:LOCALAPPDATA\Programs\Python\Python314\python.exe")){ if(Test-Path $c){$py=$c;break} }
...
Write-Host "[1/5] compiling CK_GOLD_PRO_FIX10_regime..."
...
Write-Host "[2/5] OOS regime OFF (=FIX09)..."; RunBT "false" (Join-Path $research "reg_off.csv")
Write-Host "[3/5] OOS regime ON..."; RunBT "true" (Join-Path $research "reg_on.csv")
Write-Host "[4/5] metrics..."
Push-Location $research
Write-Host ""
Write-Host "=====FIX10_REGIME_OOS_START====="
Write-Host "--- regime OFF (=FIX09) OOS 2022-25 ---"; & $py "metrics.py" "reg_off.csv" 5000
Write-Host "--- regime ON  OOS 2022-25 ---"; & $py "metrics.py" "reg_on.csv" 5000
Pop-Location
Write-Host "=====FIX10_REGIME_OOS_END====="
Write-Host "^ Copy this block. Gate helps only if ON clearly improves expectancy AND cuts DD (SPEC 4)."
