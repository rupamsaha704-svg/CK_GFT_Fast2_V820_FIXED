...
void CheckClosedTrades()
{
   ...
   for(int i = gLastDealsCount; i < totalDeals; i++)
   {
      ...
      gCurrentTicket = 0;
      SaveDailyState();
      Print("Trade closed. Net P/L: ", netProfit, " Consecutive losses: ", gConsecutiveLosses);
   }
...
void OpenTrade(ENUM_ORDER_TYPE orderType)
{
   ...
   if(sent && result.retcode == TRADE_RETCODE_DONE)
   {
      ...
      gTradesToday++;
      SaveDailyState();
      Print("Trade opened: ", (orderType == ORDER_TYPE_BUY ? "BUY" : "SELL"), " Lots: ", lots);
   }
   else
   {
...
void ShowStatus()
{
   ...
   if(gTradingAllowed) status = "READY";
   
   Comment("=== Institutional Gold Trader Pro v2.0 ===",
           "\nStatus: ", status, " | ", gBlockReason,
