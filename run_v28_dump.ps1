...
$exp=Join-Path $dataDir "MQL5\Experts"; $common=Join-Path $dataDir "..\Common\Files"; $csv=Join-Path $common "ck_v28_trades.csv"

Write-Host "[1/4] downloading + compiling v28 (v23 + block loss-hours)..."
Invoke-WebRequest "$base/CK_GFT_Fast_v28_ROBUST.mq5" -OutFile (Join-Path $exp "CK_GFT_Fast_v28_ROBUST.mq5")
$mq=Join-Path $exp "CK_GFT_Fast_v28_ROBUST.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
