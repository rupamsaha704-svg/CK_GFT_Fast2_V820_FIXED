...
//==================== OnTester: dump closed trades (ANALYSIS-ONLY) =================
// Added for MT5-truth validation ONLY. Writes entry/exit-time,profit per closed OUT
// deal to Common\Files\ck_ictchoch_trades.csv so the repo's metrics.py / pipeline.py
// can read the real Strategy-Tester result. This does NOT change any trading logic.
double OnTester()
{
   int h = FileOpen("ck_ictchoch_trades.csv", FILE_WRITE | FILE_CSV | FILE_ANSI | FILE_COMMON, ',');
   if(h != INVALID_HANDLE)
   {
