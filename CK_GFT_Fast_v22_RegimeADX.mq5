//+------------------------------------------------------------------+
//|                                    CK_GFT_Fast_v22_RegimeADX.mq5 |
//|  Base: v22 - IDENTICAL logic + ONE addition: ADX regime filter.  |
//|                                                                    |
//|  WHY: v22 is a breakout system. In ranging/choppy markets its     |
//|  breakouts fail -> SL hits far outnumber TP hits (diag: 994 SL    |
//|  vs 237 TP) and some periods lose heavily (-37%).                 |
//|  FIX: only ARM setups when ADX says the market is TRENDING.       |
//|  In ranging conditions (ADX < threshold) NO new trade is armed,   |
//|  which cuts the false-breakout SL losses.                         |
//|  Toggle: InpUseADXFilter (default true). Set false = exact v22.   |
...
//+------------------------------------------------------------------+
#property copyright "CK GFT Fast v22 RegimeADX"
...
//=== ADX REGIME FILTER (NEW - skip ranging markets) ===
input bool   InpUseADXFilter     = true;   // ON = only trade when trending; false = exact v22
input int    InpADXPeriod        = 14;     // ADX period
input double InpADXThreshold     = 20.0;   // ADX >= this => trending (try 18-28 in tester)
...
//=== HANDLES / STATE ===
int      atrHandle, emaFastHandle, emaSlowHandle, adxHandle;
...
//=== ADX regime helpers ===
double ADXMain(int shift)
{
   double b[]; if(CopyBuffer(adxHandle,0,shift,1,b)<=0) return(0); return(b[0]);
}
// Trending when ADX main line on the last closed bar >= threshold.
bool IsTrendingRegime()
{
   if(!InpUseADXFilter) return(true);      // filter off -> always allowed (exact v22)
   double adx = ADXMain(1);
   if(adx <= 0) return(false);             // no data yet -> don't arm
...
void TryArmSetup()
{
   //=== REGIME FILTER: in ranging markets (low ADX) do NOT arm any setup ===
