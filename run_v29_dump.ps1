...
$exp=Join-Path $dataDir "MQL5\Experts"; $common=Join-Path $dataDir "..\Common\Files"; $csv=Join-Path $common "ck_v29_trades.csv"

Write-Host "[1/4] downloading + compiling v29 (v23 + candle-strength confirmation)..."
Invoke-WebRequest "$base/CK_GFT_Fast_v29_ROBUST.mq5" -OutFile (Join-Path $exp "CK_GFT_Fast_v29_ROBUST.mq5")
$mq=Join-Path $exp "CK_GFT_Fast_v29_ROBUST.mq5"; $ex5=[System.IO.Path]::ChangeExtension($mq,".ex5"); if(Test-Path $ex5){Remove-Item $ex5 -Force}
...
Write-Host "[3/4] backtest - REAL TICKS, M15, MaxLot 0.09, candle-strength filter (several min)..."
$ini=Join-Path $research "v29.ini"
$cfg="[Tester]`nExpert=CK_GFT_Fast_v29_ROBUST.ex5`nSymbol=XAUUSD`nPeriod=M15`nModel=4`nExecutionMode=0`nFromDate=2025.08.01`nToDate=2026.08.01`nForwardMode=0`nDeposit=5000`nCurrency=USD`nLeverage=10`nOptimization=0`nShutdownTerminal=1`nVisual=0`n[TesterInputs]`nInpRR=3.0`nInpMaxSL_ATR=2.5`nInpRiskPercent=0.5`nInpMaxLot=0.09`nInpMinBodyFrac=0.50`nInpCloseZone=0.60`n"
