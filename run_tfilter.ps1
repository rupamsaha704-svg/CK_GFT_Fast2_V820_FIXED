...
# Time/day filter A/B: v23_tfilter OFF (=baseline) vs ON (block hours 4,6,13,15 + Tue,Thu), risk 1.7%.
# In-sample only. If ON helps, it MUST then be checked on OTHER years (2015-2018) or it is overfit.
...
Write-Host "[2] filter OFF (= baseline)..."; RunBT "false" (Join-Path $research "tf_off.csv")
Write-Host "[3] filter ON (block 4,6,13,15h + Tue,Thu)..."; RunBT "true" (Join-Path $research "tf_on.csv")
...
$off=Metrics (Join-Path $research "tf_off.csv"); $on=Metrics (Join-Path $research "tf_on.csv")
Write-Host "=====TFILTER_START====="
if($off){ Write-Host ("OFF: trades {0,4}  net {1,8:N0}  ret {2,7:N1}%%  PF {3:N2}  WR {4:N1}%%  closedDD {5:N1}%%" -f $off.n,$off.net,$off.ret,$off.pf,$off.wr,$off.mdd) }
if($on){  Write-Host ("ON : trades {0,4}  net {1,8:N0}  ret {2,7:N1}%%  PF {3:N2}  WR {4:N1}%%  closedDD {5:N1}%%" -f $on.n,$on.net,$on.ret,$on.pf,$on.wr,$on.mdd) }
Write-Host "=====TFILTER_END====="
Write-Host "^ Copy this block. If ON is better, next step is to run the SAME filter on 2015-2018 to check it is not overfit."
